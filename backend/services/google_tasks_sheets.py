...
# 업무정리 탭 열 매핑 (1-based → 0-based index)
# A=완료, B=마감일, C=관련플랫폼, D=분류, E=우선순위, F=업무명,
# G=난이도, H=피로도, I=상태, J=담당자/요청주체, K=관련작품, L=메모
_COLS = 12
_IDX = dict(완료=0, 마감일=1, 관련플랫폼=2, 분류=3, 우선순위=4, 업무명=5,
            난이도=6, 피로도=7, 상태=8, 담당자=9, 관련작품=10, 메모=11)
...
def fetch_tasks(settings: Settings) -> list[dict]:
    ...
    esc = _tab_esc(tab)
    rows = read_sheet_tab_values(cred, sid, f"'{esc}'!A2:L")
    out = []
...
def create_task(settings: Settings, fields: dict) -> dict:
    ...
    row = [""] * _COLS
    row[_IDX["업무명"]] = title
    for key in ("마감일", "관련플랫폼", "분류", "우선순위", "난이도", "피로도", "상태", "담당자", "관련작품", "메모"):
        val = str(fields.get(key, "")).strip()
        if val:
            row[_IDX[key]] = val
    esc = _tab_esc(tab)
    from .google_sheets import append_rows_to_sheet_range as _append
    import re
    updated = _append(cred, sid, f"'{esc}'!A:L", [row])
    m = re.search(r"!([A-Za-z]+)(\d+)", updated or "")
    sheet_row = int(m.group(2)) if m else 0
    return {"id": _row_id(sheet_row), "sheet_row": sheet_row, "업무명": title, **{k: str(fields.get(k, "")) for k in ("마감일", "관련플랫폼", "분류", "우선순위", "난이도", "피로도", "상태", "담당자", "관련작품", "메모")}}
...
def update_task(settings: Settings, task_id: str, fields: dict) -> None:
    ...
    esc = _tab_esc(tab)
    col_map = {"완료": "A", "마감일": "B", "관련플랫폼": "C", "분류": "D", "우선순위": "E",
               "업무명": "F", "난이도": "G", "피로도": "H", "상태": "I", "담당자": "J", "관련작품": "K", "메모": "L"}
    data = []
