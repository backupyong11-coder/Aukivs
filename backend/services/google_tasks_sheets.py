...
_TASK_HEADER_ALIASES: dict[str, tuple[str, ...]] = {
    "정량화 분": ("정량화 분", "정량화 분류"),
    "실행일": ("실행일",),
    "외부담당자": ("외부담당자", "업무담당", "인물담당", "상태"),
    "담당자": ("담당자", "피로도", "담당자/요청주체"),
}
...
def fetch_tasks(settings: Settings) -> list[dict]:
    ...
    for i, row in enumerate(all_rows[1:], start=2):
        ...
        out.append({
            ...
            "실행일": _c(cells, "실행일", col_map),
            "분야": _c(cells, "분야", col_map),
            "분류": _c(cells, "분류", col_map),
            "대분류": _c(cells, "대분류", col_map),
...
def create_task(settings: Settings, fields: dict) -> dict:
    ...
    if not str(out.get("외부담당자", "")).strip():
        legacy = str(fields.get("업무담당", "") or fields.get("인물담당", "") or fields.get("상태", "")).strip()
        if legacy:
            out["외부담당자"] = legacy
    return out
