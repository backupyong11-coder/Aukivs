"""유통 대시보드: Supabase `public.distribution_documents` (단일 JSON 문서)."""
...
def require_distribution_supabase(settings: Settings) -> None:
    if not (settings.supabase_url or "").strip() or not (
        settings.supabase_service_role_key or ""
    ).strip():
        raise SupabaseConfigurationError(
            "[설정] 유통 대시보드 서버 저장에는 SUPABASE_URL 과 "
...
def _validate_profile(profile: dict[str, Any]) -> None:
    if profile.get("version") != 1:
        raise ValueError("[파싱] distribution profile version 은 1 이어야 합니다.")
...
def get_profile(settings: Settings) -> dict[str, Any] | None:
    require_distribution_supabase(settings)
