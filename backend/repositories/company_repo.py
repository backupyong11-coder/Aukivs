"""회사 대시보드: Supabase `public.company_profile_documents` (단일 JSON 문서)."""

from __future__ import annotations

from typing import Any

from config import Settings
from services.supabase_client import SupabaseConfigurationError, SupabaseRestClient

DOCUMENT_ID = "default"


def _client(settings: Settings) -> SupabaseRestClient:
    url = (settings.supabase_url or "").strip()
    key = (settings.supabase_service_role_key or "").strip()
    return SupabaseRestClient(url, key)


def require_company_supabase(settings: Settings) -> None:
    if not (settings.supabase_url or "").strip() or not (
        settings.supabase_service_role_key or ""
    ).strip():
        raise SupabaseConfigurationError(
            "[설정] 회사 대시보드 서버 저장에는 SUPABASE_URL 과 "
            "SUPABASE_SERVICE_ROLE_KEY 가 필요합니다."
        )


def _validate_profile(profile: dict[str, Any]) -> None:
    if profile.get("version") != 1:
        raise ValueError("[파싱] 회사 profile version 은 1 이어야 합니다.")
    if not isinstance(profile.get("companyName"), str):
        raise ValueError("[파싱] companyName 이 필요합니다.")
    sections = profile.get("sections")
    if not isinstance(sections, list) or len(sections) == 0:
        raise ValueError("[파싱] sections 가 비어 있습니다.")
    for item in sections:
        if not isinstance(item, dict):
            raise ValueError("[파싱] sections 항목 형식이 올바르지 않습니다.")
        if not isinstance(item.get("columns"), list) or not isinstance(item.get("rows"), list):
            raise ValueError("[파싱] section columns/rows 가 필요합니다.")


def get_profile(settings: Settings) -> dict[str, Any] | None:
    require_company_supabase(settings)
    cli = _client(settings)
    rows = cli.get_json(
        "/company_profile_documents",
        params={
            "id": f"eq.{DOCUMENT_ID}",
            "select": "profile,updated_at",
            "limit": "1",
        },
    )
    if not isinstance(rows, list) or len(rows) == 0:
        return None
    row = rows[0]
    if not isinstance(row, dict):
        return None
    prof = row.get("profile")
    if not isinstance(prof, dict):
        return None
    _validate_profile(prof)
    return {
        "profile": prof,
        "updated_at": row.get("updated_at"),
    }


def upsert_profile(settings: Settings, profile: dict[str, Any]) -> str | None:
    require_company_supabase(settings)
    _validate_profile(profile)
    cli = _client(settings)
    body = {"id": DOCUMENT_ID, "profile": profile}
    result = cli.post_json(
        "/company_profile_documents",
        body,
        prefer="resolution=merge-duplicates,return=representation",
    )
    if isinstance(result, list) and len(result) > 0 and isinstance(result[0], dict):
        ua = result[0].get("updated_at")
        return str(ua) if ua is not None else None
    existing = get_profile(settings)
    if existing:
        ua = existing.get("updated_at")
        return str(ua) if ua is not None else None
    return None
