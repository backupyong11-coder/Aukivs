...
import {
  normalizeProductionProcessProfile,
  type ProductionProcessProfile,
} from "@/lib/productionProcessStorage";

export type FetchProductionProcessResult =
  | { ok: true; profile: ProductionProcessProfile | null; updatedAt: string | null }
  | { ok: false; message: string; status?: number };

export type SaveProductionProcessResult =
...
function userFacingError(status: number, raw: string, op: "fetch" | "save"): string {
  ...
  if (status === 503) {
    return op === "fetch"
      ? "제작공정 서버(Supabase) 설정을 확인할 수 없습니다."
      : "제작공정을 서버에 저장할 수 없습니다. Supabase 설정을 확인하세요.";
  }
  if (status >= 500) {
    return "제작공정 서버와 통신하지 못했습니다. 잠시 후 다시 시도하세요.";
  }
  try {
...
export async function fetchProductionProcessFromServer(): Promise<FetchProductionProcessResult> {
  const base = getApiBaseUrl();
  try {
    const res = await fetch(`${base}/production-process`, {
      method: "GET",
      credentials: "include",
      cache: "no-store",
    });
    ...
    const profile = normalizeProductionProcessProfile(j.profile);
    if (!profile) {
      return { ok: false, message: "서버에 저장된 제작공정 형식이 올바르지 않습니다." };
    }
    return { ok: true, profile, updatedAt: j.updated_at ?? null };
  } catch {
    return { ok: false, message: "제작공정 서버에 연결할 수 없습니다." };
  }
}

export async function saveProductionProcessToServer(
  profile: ProductionProcessProfile,
): Promise<SaveProductionProcessResult> {
  const base = getApiBaseUrl();
  try {
    const res = await fetch(`${base}/production-process`, {
      method: "PUT",
