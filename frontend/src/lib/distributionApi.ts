...
export type FetchDistributionResult =
  | { ok: true; profile: DistributionProfile | null; updatedAt: string | null }
  | { ok: false; message: string; status?: number };

export type SaveDistributionResult =
...
function userFacingError(status: number, raw: string, op: "fetch" | "save"): string {
  ...
  if (status === 503) {
    return op === "fetch"
      ? "유통 서버(Supabase) 설정을 확인할 수 없습니다."
      : "유통을 서버에 저장할 수 없습니다. Supabase 설정을 확인하세요.";
  }
  if (status >= 500) {
    return "유통 서버와 통신하지 못했습니다. 잠시 후 다시 시도하세요.";
  }
  try {
...
export async function fetchDistributionFromServer(): Promise<FetchDistributionResult> {
  ...
  try {
    ...
    const profile = normalizeDistributionProfile(j.profile);
    if (!profile) {
      return { ok: false, message: "서버에 저장된 유통 형식이 올바르지 않습니다." };
    }
    return { ok: true, profile, updatedAt: j.updated_at ?? null };
  } catch {
    return { ok: false, message: "유통 서버에 연결할 수 없습니다." };
  }
}

export async function saveDistributionToServer(
  profile: DistributionProfile,
): Promise<SaveDistributionResult> {
  const base = getApiBaseUrl();
  try {
    const res = await fetch(`${base}/distribution`, {
      method: "PUT",
