    ...
    if (id === "platform_status") {
      openPanel({
        kind: "render", title: "플랫폼 마지막 상황", node: (
          <ul className="grid max-h-80 grid-cols-2 gap-2 overflow-y-auto">{hub.platformMaster.filter(p => (p["마지막상황"] || p["마지막 상황"]) && !isTrue(p["예정"])).map((p, i) => (
            <li key={i} className="min-w-0 rounded-lg border border-zinc-200 bg-zinc-50/90 px-3 py-2 text-xs break-words">
    ...
    if (id === "platform_waiting") {
      openPanel({
        kind: "render", title: "플랫폼 대기사유", node: (
          <ul className="grid max-h-80 grid-cols-2 gap-2 overflow-y-auto">{hub.platformMaster.filter(p => p["대기사유"] && p["대기사유"] !== "아직 없음").map((p, i) => (
            <li key={i} className="min-w-0 rounded-lg border border-zinc-200 bg-zinc-50/90 px-3 py-2 text-xs break-words dark:border-zinc-700 dark:bg-zinc-900/50">
              <span className="font-semibold">{p["회사명"] ?? ""}</span>
    ...
    if (id === "platform_action") {
      openPanel({
        kind: "render", title: "플랫폼 다음액션", node: (
          <ul className="grid max-h-80 grid-cols-2 gap-2 overflow-y-auto">{hub.platformMaster.filter(p => p["다음액션"] && p["다음액션"] !== "아직 없음" && !isTrue(p["불가"]) && !isTrue(p["예정"])).map((p, i) => (
            <li key={i} className="min-w-0 rounded-lg border border-zinc-200 bg-zinc-50/90 px-3 py-2 text-xs break-words">
              <span className="font-semibold">{p["회사명"] ?? ""}</span>
      ...
      <header className="border-b border-zinc-200 bg-white px-4 py-3 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="mx-auto mt-3 max-w-[1600px] border-t border-zinc-100 pt-3 dark:border-zinc-800">
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" className={quickBtn} onClick={() => void runPreset("platform_status", "마지막 상황")}>마지막 상황</button>
