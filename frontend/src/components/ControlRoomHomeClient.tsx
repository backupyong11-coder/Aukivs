...
export function ControlRoomHomeClient() {
    ...
    async (id: string, labelForRecent?: string) => {
      ...
      if (hub.kind !== "ready") {
        openPanel({
          ...
          message:
            hub.kind === "error"
              ? hub.message
              : "아직 관제 데이터를 불러오는 중입니다. 잠시 후 다시 시도하세요.",
        });
        return;
      }
          ...
          node: (
            <div className="space-y-3 text-sm text-zinc-800 dark:text-zinc-200">
              <p className="leading-relaxed text-zinc-600 dark:text-zinc-400">
                ...
                건. 긴급 후보 중 체크 출처는 아래에 미리 보여 줍니다. 수정은{" "}
      ...
      if (id === "today_upload") {
        ...
        openPanel({
          ...
          node: (
            <UploadPreviewList
              items={rows}
              empty="오늘 날짜(D열)로 잡힌 업로드 행이 없습니다."
              actionHref="/uploads"
              actionLabel="업로드 작업에서 전체 보기"
            />
          ),
        });
        return;
      ...
      if (id === "week_upload") {
        ...
        openPanel({
          ...
          node: (
            <UploadPreviewList
              items={rows}
              empty="이번 주 업로드 시각(D열)으로 잡힌 행이 없습니다."
              actionHref="/uploads"
              actionLabel="업로드 작업에서 전체·필터"
            />
          ),
        });
        return;
        ...
        openPanel({
          ...
          node: (
            <UploadPreviewList
              ...
              empty="상태가 비었거나 완료로 보이지 않는 행이 없습니다."
              actionHref="/uploads"
              actionLabel="업로드 작업에서 처리"
            />
          ),
        });
        return;
            ...
            <div className="space-y-3 text-sm">
              {dup.length === 0 ? (
                ...
                <ul className="list-inside list-disc space-y-1 text-zinc-800 dark:text-zinc-200">
                  {dup.map((iss, i) => (
                    <li key={`${iss.id}-${i}`}>
                      <span className="font-mono text-xs">{iss.id}</span> — 행{" "}
        ...
        openPanel({
          ...
          node: (
            <p className="text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
              특정 플랫폼·작품만 걸러 보는 조회는 다음 턴에서 시트 열·API와 연결합니다.
              좌측 선택 상자가 활성화되면 여기서 전체정보를 띄웁니다.
            </p>
          ),
        });
        return;
      ...
      if (id === "sheet_backup") {
        openPanel({
          ...
          node: (
            <p className="text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
              Google 스프레드시트 메뉴에서 사본 만들기·버전 기록을 사용하거나, 다음 단계에서
              서버 백업 API를 연결합니다.
            </p>
          ),
        });
        ...
        openPanel({
          ...
          node: (
            <div className="space-y-3 text-sm text-zinc-800 dark:text-zinc-200">
              ...
              <MemoPreviewList
                items={hub.memos}
                emptyHint="표시할 메모가 없습니다. 왼쪽 사이드바에서 메모를 추가하거나 시트를 확인하세요."
              />
              <p className="text-xs text-zinc-500 dark:text-zinc-400">
                분류는 시트 「메모분류」 열에서 입력하면, 질문하기 검색에
                포함됩니다.
              </p>
            </div>
          ),
        ...
        openPanel({
          ...
          node: (
            <div className="space-y-3 text-sm text-zinc-800 dark:text-zinc-200">
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400">
                검색어 「{q}」 — 메모내용·메모분류에 공백으로 나눈 키워드가 모두
                들어 있는 행만 표시합니다.
              </p>
              <MemoPreviewList
                items={matches}
                emptyHint="일치하는 메모가 없습니다. 키워드를 줄이거나 시트에서 분류를 입력한 뒤 상단「전체 새로고침」을 누르세요."
              />
              <p className="border-t border-zinc-100 pt-2 text-xs text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
                체크·업로드·브리핑은 상단 빠른 조회 버튼을 사용하세요.
              </p>
            </div>
          ),
        });
  ...
  const copyResultPanel = useCallback(async () => {
    ...
    try {
      ...
      window.alert("복사에 실패했습니다. 브라우저 클립보드 권한을 확인하세요.");
    }
  }, []);
    ...
    <div className="min-h-full bg-zinc-100/90 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-50">
      <header className="border-b border-zinc-200 bg-white px-4 py-3 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="mx-auto mt-3 max-w-[1600px] space-y-2 border-t border-zinc-100 pt-3 dark:border-zinc-800">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" className={quickBtn} onClick={() => void runPreset("today_upload", "오늘 업로드")}>
              오늘 업로드
            </button>
            <button type="button" className={quickBtn} onClick={() => void runPreset("week_upload", "이번 주 업로드")}>
              이번 주 업로드
            </button>
            <button type="button" className={quickBtn} onClick={() => void runPreset("upload_gaps", "미완료 업로드")}>
              미완료 업로드
            </button>
            <button type="button" className={quickBtn} onClick={() => void runPreset("upload_summary", "업로드 요약")}>
              업로드 요약
            </button>
            <button type="button" className={quickBtn} onClick={() => void runPreset("today_triage", "오늘 브리핑")}>
              오늘 브리핑
            </button>
          </div>
          <p className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" className={quickBtn} onClick={() => void runPreset("due_today", "오늘 마감")}>
              오늘 마감
            </button>
            <button type="button" className={quickBtn} onClick={() => void runPreset("incomplete_check", "미완료 업무")}>
              미완료 업무
            </button>
            <button type="button" className={quickBtn} onClick={() => void runPreset("urgent_only", "급한 일")}>
              급한 일
            </button>
          </div>
          <p className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" className={quickBtn} onClick={() => void runPreset("data_bad", "데이터 점검")}>
              데이터 점검
            </button>
            <button type="button" className={quickBtn} onClick={() => void runPreset("sheet_backup", "시트 백업")}>
              시트 백업
            </button>
            <button
              type="button"
        ...
        <aside className="space-y-3 lg:col-span-2">
          <section className="rounded-lg border border-zinc-200 bg-white p-3 text-sm dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <button
              ...
              onClick={() => void runPreset("platform_stub", "전체정보 보기")}
            >
              전체정보 보기
            </button>
          </section>
          ...
          <section className="rounded-lg border border-zinc-200 bg-white p-3 text-xs dark:border-zinc-800 dark:bg-zinc-950">
            ...
            {recent.length === 0 ? (
              <p className="mt-2 text-zinc-500">없음</p>
            ) : (
              <ul className="mt-2 max-h-40 space-y-1 overflow-y-auto">
                {recent.map((q) => (
                  <li key={q} className="flex gap-1">
                    ...
                    <button type="button" className="text-amber-600" title="즐겨찾기" onClick={() => { toggleFavoriteQuery(q); refreshHistory(); }}>
              ...
              {panel.kind === "nl_stub" ? (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-zinc-500">
                    관제 데이터가 준비되면 같은 질문은 메모 검색에 사용됩니다. 지금은
                    상단 빠른 조회를 이용하세요.
                  </p>
                </div>
              ) : null}
        ...
        >
          <p className="text-[10px] text-zinc-500 dark:text-zinc-400">
            ...
            {m.category ? (
              ...
              <span className="ml-2 text-zinc-400">분류 없음</span>
            )}
          </p>
...
function ChecklistPreviewList(props: {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-zinc-500 dark:text-zinc-400">
        활성 행 {props.total}건 중 {props.items.length}건 미리보기
      </p>
      <ul className="max-h-64 space-y-2 overflow-y-auto">
        {props.items.map((it) => (
          <li
      ...
      {props.dup.length > 0 ? (
        <div>
          ...
          <ul className="mt-1 space-y-1 text-zinc-800 dark:text-zinc-200">
            {props.dup.map((d, i) => (
              <li key={`d-${d.id}-${i}`}>
                <span className="font-mono text-xs">{d.id}</span> — 행{" "}
                {d.sheet_rows.join(", ")}
              </li>
            ))}
          </ul>
